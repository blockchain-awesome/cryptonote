...
public:
  ...
  virtual std::list<peerlist_entry_t> getLocalPeerList() const = 0;
  virtual basic_node_data getNodeData() const = 0;
  virtual peer_id_type_t getPeerId() const = 0;

  virtual void handleNodeData(const basic_node_data& node, P2pContext& ctx) = 0;
  virtual bool handleRemotePeerList(const std::list<peerlist_entry_t>& peerlist, time_t local_time) = 0;
