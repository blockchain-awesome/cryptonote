...
namespace cryptonote
{
  ...
  P2pNode::P2pNode(const P2pNodeConfig &cfg, Dispatcher &dispatcher, Logging::ILogger &log, const hash_t &genesisHash, peer_id_type_t peerId) : logger(log, "P2pNode:" + std::to_string(cfg.getBindPort())),
  ...
  bool P2pNode::makeNewConnectionFromPeerlist(const PeerlistManager::Peerlist &peerlist)
  {
    ...
    for (size_t tryCount = 0; idxGen.generate(peerIndex) && tryCount < m_cfg.getPeerListGetTryCount(); ++tryCount)
    {
      ...
      logger(DEBUGGING) << "Selected peer: [" << peer.id << " " << peer.adr << "] last_seen: " << (peer.last_seen ? ::string::Time::ago(time(NULL) - peer.last_seen) : "never");
