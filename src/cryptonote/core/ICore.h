...
public:
  ...
  virtual void get_blockchain_top(uint32_t& height, hash_t& top_id) = 0;
  ...
  virtual bool get_tx_outputs_gindexs(const hash_t& tx_id, std::vector<uint32_t>& indexs) = 0;
  ...
  virtual hash_t getBlockIdByHeight(uint32_t height) = 0;
  virtual bool getBlockByHash(const hash_t &h, block_t &blk) = 0;
  virtual void getTransactions(const std::vector<hash_t>& txs_ids, std::list<transaction_t>& txs, std::list<hash_t>& missed_txs, bool checkTxPool = false) = 0;
  virtual bool getBackwardBlocksSizes(uint32_t fromHeight, std::vector<size_t>& sizes, size_t count) = 0;
  virtual bool getBlockSize(const hash_t& hash, size_t& size) = 0;
  virtual bool getAlreadyGeneratedCoins(const hash_t& hash, uint64_t& generatedCoins) = 0;
  ...
  virtual bool scanOutputkeysForIndices(const key_input_t& txInToKey, std::list<std::pair<hash_t, size_t>>& outputReferences) = 0;
  virtual bool getBlockDifficulty(uint32_t height, difficulty_t& difficulty) = 0;
  virtual bool getBlockContainingTx(const hash_t& txId, hash_t& blockId, uint32_t& blockHeight) = 0;
  virtual bool getMultisigOutputReference(const multi_signature_input_t& txInMultisig, std::pair<hash_t, size_t>& outputReference) = 0;
  ...
  virtual bool getTransactionsByPaymentId(const hash_t& paymentId, std::vector<transaction_t>& transactions) = 0;

  virtual std::unique_ptr<IBlock> getBlock(const hash_t& blocksId) = 0;
  virtual bool handleIncomingTransaction(const transaction_t& tx, const hash_t& txHash, size_t blobSize, tx_verification_context_t& tvc, bool keptByBlock) = 0;
