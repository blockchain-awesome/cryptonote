...
class ITransactionReader
{
  public:
    ...
    virtual hash_t getTransactionHash() const = 0;
    virtual hash_t getTransactionPrefixHash() const = 0;
    virtual public_key_t getTransactionPublicKey() const = 0;
    virtual bool getTransactionSecretKey(secret_key_t &key) const = 0;
    virtual uint64_t getUnlockTime() const = 0;

    // extra
    virtual bool getPaymentId(hash_t &paymentId) const = 0;
    ...
    virtual input_type_t getInputType(size_t index) const = 0;
    ...
    virtual output_type_t getOutputType(size_t index) const = 0;
    ...
    virtual bool findOutputsToAccount(const account_public_address_t &addr, const secret_key_t &viewSecretKey, std::vector<uint32_t> &outs, uint64_t &outputAmount) const = 0;
