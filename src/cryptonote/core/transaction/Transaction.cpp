...
namespace cryptonote
{
  ...
  {
    return std::accumulate(transaction.inputs.begin(), transaction.inputs.end(), 0ULL, [](uint64_t val, const transaction_input_t &in) { return val + getTransactionInputAmount(in); });
  }
  ...
  {
    return std::accumulate(transaction.outputs.begin(), transaction.outputs.end(), 0ULL, [](uint64_t val, const transaction_output_t &out) { return val + out.amount; });
  }
