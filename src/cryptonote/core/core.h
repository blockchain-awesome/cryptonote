...
namespace cryptonote {
  ...
  class core : public ICore, public IMinerHandler, public IBlockchainStorageObserver, public ITxPoolObserver {
   public:
     ...
     virtual bool getBlockSize(const hash_t& hash, size_t& size) override;
     virtual bool getAlreadyGeneratedCoins(const hash_t& hash, uint64_t& generatedCoins) override;
     ...
     virtual bool scanOutputkeysForIndices(const key_input_t& txInToKey, std::list<std::pair<hash_t, size_t>>& outputReferences) override;
     virtual bool getBlockDifficulty(uint32_t height, difficulty_t& difficulty) override;
     virtual bool getBlockContainingTx(const hash_t& txId, hash_t& blockId, uint32_t& blockHeight) override;
     virtual bool getMultisigOutputReference(const multi_signature_input_t& txInMultisig, std::pair<hash_t, size_t>& output_reference) override;
     ...
     virtual bool getTransactionsByPaymentId(const hash_t& paymentId, std::vector<transaction_t>& transactions) override;
     virtual bool getOutByMSigGIndex(uint64_t amount, uint64_t gindex, multi_signature_output_t& out) override;
     virtual std::unique_ptr<IBlock> getBlock(const hash_t& blocksId) override;
     virtual bool handleIncomingTransaction(const transaction_t& tx, const hash_t& txHash, size_t blobSize, tx_verification_context_t& tvc, bool keptByBlock) override;
     ...
     bool have_block(const hash_t& id) override;
     ...
     virtual void get_blockchain_top(uint32_t& height, hash_t& top_id) override;
     ...
     virtual bool queryBlocks(const std::vector<hash_t>& block_ids, uint64_t timestamp,
       uint32_t& start_height, uint32_t& current_height, uint32_t& full_offset, std::vector<block_full_info_t>& entries) override;
    virtual bool queryBlocksLite(const std::vector<hash_t>& knownBlockIds, uint64_t timestamp,
      uint32_t& resStartHeight, uint32_t& resCurrentHeight, uint32_t& resFullOffset, std::vector<block_short_info_t>& entries) override;
    virtual hash_t getBlockIdByHeight(uint32_t height) override;
    void getTransactions(const std::vector<hash_t>& txs_ids, std::list<transaction_t>& txs, std::list<hash_t>& missed_txs, bool checkTxPool = false) override;
    virtual bool getBlockByHash(const hash_t &h, block_t &blk) override;
     ...
     bool get_stat_info(core_state_info_t& st_inf) override;
     ...
     virtual bool getPoolChanges(const hash_t& tailBlockId, const std::vector<hash_t>& knownTxsIds,
                                 std::vector<transaction_t>& addedTxs, std::vector<hash_t>& deletedTxsIds) override;
     virtual bool getPoolChangesLite(const hash_t& tailBlockId, const std::vector<hash_t>& knownTxsIds,
                                  std::vector<transaction_prefix_info_t>& addedTxs, std::vector<hash_t>& deletedTxsIds) override;
     virtual void getPoolChanges(const std::vector<hash_t>& knownTxsIds, std::vector<transaction_t>& addedTxs,
                                 std::vector<hash_t>& deletedTxsIds) override;

     uint64_t getNextBlockDifficulty();
     uint64_t getTotalGeneratedAmount();

   private:
     bool add_new_tx(const transaction_t& tx, const hash_t& tx_hash, size_t blob_size, tx_verification_context_t& tvc, bool keeped_by_block);
     bool load_state_data();
     bool parse_tx_from_blob(transaction_t& tx, hash_t& tx_hash, hash_t& tx_prefix_hash, const binary_array_t& blob);
     ...
     bool is_key_image_spent(const key_image_t& key_im);

     bool check_tx_ring_signature(const key_input_t& tx, const hash_t& tx_prefix_hash, const std::vector<signature_t>& sig);
     ...
     bool findStartAndFullOffsets(const std::vector<hash_t>& knownBlockIds, uint64_t timestamp, uint32_t& startOffset, uint32_t& startFullOffset);
     std::vector<hash_t> findIdsForShortBlocks(uint32_t startOffset, uint32_t startFullOffset);
