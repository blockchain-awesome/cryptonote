...
SingleWallet::SingleWallet(const CryptoNote::Currency& currency, INode& node) :
...
void SingleWallet::addObserver(IWalletLegacyObserver* observer) {
  m_observerManager.add(observer);
}

void SingleWallet::removeObserver(IWalletLegacyObserver* observer) {
  m_observerManager.remove(observer);
}

void SingleWallet::initAndGenerate(const std::string& password) {
  {
...
void SingleWallet::initAndLoad(std::istream& source, const std::string& password) {
  std::unique_lock<std::mutex> stateLock(m_cacheMutex);
  ...
  std::thread loader(&SingleWallet::doLoad, this, std::ref(source));
  loader.detach();
}
...
void SingleWallet::doLoad(std::istream& source) {
...
void SingleWallet::save(std::ostream& destination, bool saveDetailed, bool saveCache) {
  if(m_isStopping) {
  ...
  m_asyncContextCounter.addAsyncContext();
  std::thread saver(&SingleWallet::doSave, this, std::ref(destination), saveDetailed, saveCache);
  saver.detach();
}

void SingleWallet::doSave(std::ostream& destination, bool saveDetailed, bool saveCache) {
  ContextCounterHolder counterHolder(m_asyncContextCounter);
...
std::error_code SingleWallet::changePassword(const std::string& oldPassword, const std::string& newPassword) {
...
uint64_t SingleWallet::actualBalance() {
  std::unique_lock<std::mutex> lock(m_cacheMutex);
  throwIfNotInitialised();
...
uint64_t SingleWallet::pendingBalance() {
  std::unique_lock<std::mutex> lock(m_cacheMutex);
  throwIfNotInitialised();
...
size_t SingleWallet::getTransactionCount() {
  std::unique_lock<std::mutex> lock(m_cacheMutex);
  throwIfNotInitialised();
...
size_t SingleWallet::getTransferCount() {
  std::unique_lock<std::mutex> lock(m_cacheMutex);
  throwIfNotInitialised();
...
TransactionId SingleWallet::findTransactionByTransferId(TransferId transferId) {
  std::unique_lock<std::mutex> lock(m_cacheMutex);
  throwIfNotInitialised();
...
bool SingleWallet::getTransaction(TransactionId transactionId, WalletLegacyTransaction& transaction) {
  std::unique_lock<std::mutex> lock(m_cacheMutex);
  throwIfNotInitialised();
...
bool SingleWallet::getTransfer(TransferId transferId, WalletLegacyTransfer& transfer) {
  std::unique_lock<std::mutex> lock(m_cacheMutex);
  throwIfNotInitialised();
...
TransactionId SingleWallet::sendTransaction(const WalletLegacyTransfer& transfer, uint64_t fee, const std::string& extra, uint64_t mixIn, uint64_t unlockTimestamp) {
...
TransactionId SingleWallet::sendTransaction(const std::vector<WalletLegacyTransfer>& transfers, uint64_t fee, const std::string& extra, uint64_t mixIn, uint64_t unlockTimestamp) {
  ...
  if (request) {
    m_asyncContextCounter.addAsyncContext();
    request->perform(m_node, std::bind(&SingleWallet::sendTransactionCallback, this, std::placeholders::_1, std::placeholders::_2));
  }

  return txId;
}

void SingleWallet::sendTransactionCallback(WalletRequest::Callback callback, std::error_code ec) {
  ContextCounterHolder counterHolder(m_asyncContextCounter);
  std::deque<std::shared_ptr<WalletLegacyEvent> > events;
  ...
  if (nextRequest) {
    m_asyncContextCounter.addAsyncContext();
    (*nextRequest)->perform(m_node, std::bind(&SingleWallet::synchronizationCallback, this, std::placeholders::_1, std::placeholders::_2));
  }
}

void SingleWallet::synchronizationCallback(WalletRequest::Callback callback, std::error_code ec) {
  ContextCounterHolder counterHolder(m_asyncContextCounter);
...
void SingleWallet::synchronizationCompleted(std::error_code result) {
...
void SingleWallet::onTransactionUpdated(ITransfersSubscription* object, const Hash& transactionHash) {
...
void SingleWallet::onTransactionDeleted(ITransfersSubscription* object, const Hash& transactionHash) {
  std::shared_ptr<WalletLegacyEvent> event;
...
void SingleWallet::throwIfNotInitialised() {
...
void SingleWallet::notifyClients(std::deque<std::shared_ptr<WalletLegacyEvent> >& events) {
  while (!events.empty()) {
...
void SingleWallet::notifyIfBalanceChanged() {
...
std::vector<TransactionId> SingleWallet::deleteOutdatedUnconfirmedTransactions() {
