...
void WalletInterface::clearCaches()
{
...
void WalletInterface::validateTransactionParameters(const TransactionParameters &transactionParameters)
{
...
size_t WalletInterface::doTransfer(const TransactionParameters &transactionParameters)
{
...
size_t WalletInterface::insertOutgoingTransactionAndPushEvent(const Hash &transactionHash, uint64_t fee, const BinaryArray &extra, uint64_t unlockTimestamp)
{
...
void WalletInterface::updateTransactionStateAndPushEvent(size_t transactionId, WalletTransactionState state)
{
...
bool WalletInterface::updateWalletTransactionInfo(size_t transactionId, const CryptoNote::TransactionInformation &info, int64_t totalAmount)
{
...
size_t WalletInterface::insertBlockchainTransaction(const TransactionInformation &info, int64_t txBalance)
{
...
bool WalletInterface::updateTransactionTransfers(size_t transactionId, const std::vector<ContainerAmounts> &containerAmountsList,
...
WalletInterface::TransfersMap WalletInterface::getKnownTransfersMap(size_t transactionId, size_t firstTransferIdx) const
{
  TransfersMap result;
...
bool WalletInterface::updateAddressTransfers(size_t transactionId, size_t firstTransferIdx, const std::string &address, int64_t knownAmount, int64_t targetAmount)
{
...
bool WalletInterface::updateUnknownTransfers(size_t transactionId, size_t firstTransferIdx, const std::unordered_set<std::string> &myAddresses,
...
void WalletInterface::appendTransfer(size_t transactionId, size_t firstTransferIdx, const std::string &address, int64_t amount)
{
...
bool WalletInterface::adjustTransfer(size_t transactionId, size_t firstTransferIdx, const std::string &address, int64_t amount)
{
  assert(amount != 0);
...
bool WalletInterface::eraseTransfers(size_t transactionId, size_t firstTransferIdx, std::function<bool(bool, const std::string &)> &&predicate)
{
  bool erased = false;
  auto it = std::next(m_transfers.begin(), firstTransferIdx);
  while (it != m_transfers.end() && it->first == transactionId)
...
bool WalletInterface::eraseTransfersByAddress(size_t transactionId, size_t firstTransferIdx, const std::string &address, bool eraseOutputTransfers)
{
...
bool WalletInterface::eraseForeignTransfers(size_t transactionId, size_t firstTransferIdx, const std::unordered_set<std::string> &knownAddresses,
...
std::unique_ptr<CryptoNote::ITransaction> WalletInterface::makeTransaction(const std::vector<ReceiverAmounts> &decomposedOutputs,
...
void WalletInterface::sendTransaction(const CryptoNote::Transaction &cryptoNoteTransaction)
{
  System::Event completion(m_dispatcher);
  std::error_code ec;
...
size_t WalletInterface::validateSaveAndSendTransaction(const ITransactionReader &transaction, const std::vector<WalletTransfer> &destinations, bool isFusion, bool send)
{
...
AccountKeys WalletInterface::makeAccountKeys(const WalletRecord &wallet) const
{
  AccountKeys keys;
...
void WalletInterface::requestMixinOuts(
...
uint64_t WalletInterface::selectTransfers(
...
std::vector<WalletInterface::WalletOuts> WalletInterface::pickWalletsWithMoney() const
{
...
WalletInterface::WalletOuts WalletInterface::pickWallet(const std::string &address)
{
  const auto &wallet = getWalletRecord(address);
...
std::vector<WalletInterface::WalletOuts> WalletInterface::pickWallets(const std::vector<std::string> &addresses)
{
  std::vector<WalletOuts> wallets;
...
WalletInterface::ReceiverAmounts WalletInterface::splitAmount(
    uint64_t amount,
...
void WalletInterface::prepareInputs(
    const std::vector<OutputToTransfer> &selectedTransfers,
    std::vector<CryptoNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount> &mixinResult,
    uint64_t mixIn,
    std::vector<InputInfo> &keysInfo)
...
size_t WalletInterface::getTransactionId(const Hash &transactionHash) const
{
...
void WalletInterface::deleteUnlockTransactionJob(const Hash &transactionHash)
{
...
void WalletInterface::addUnconfirmedTransaction(const ITransactionReader &transaction)
{
...
void WalletInterface::removeUnconfirmedTransaction(const Crypto::Hash &transactionHash)
{
...
const WalletRecord &WalletInterface::getWalletRecord(const std::string &address) const
{
  CryptoNote::AccountPublicAddress pubAddr = parseAddress(address);
  return getWalletRecord(pubAddr.spendPublicKey);
}

const WalletRecord &WalletInterface::getWalletRecord(CryptoNote::ITransfersContainer *container) const
{
  auto it = m_walletsContainer.get<TransfersContainerIndex>().find(container);
  if (it == m_walletsContainer.get<TransfersContainerIndex>().end())
  {
...
CryptoNote::AccountPublicAddress WalletInterface::parseAddress(const std::string &address) const
{
...
void WalletInterface::throwIfStopped() const
{
  if (m_stopped)
  {
...
void WalletInterface::throwIfTrackingMode() const
{
...
WalletInterface::WalletTrackingMode WalletInterface::getTrackingMode() const
{
...
// size_t WalletInterface::createFusionTransaction(uint64_t threshold, uint64_t mixin)
// {
//   Tools::ScopeExit releaseContext([this] {
//     m_dispatcher.yield();
//   });
...
WalletInterface::ReceiverAmounts WalletInterface::decomposeFusionOutputs(uint64_t inputsAmount)
{
  assert(m_walletsContainer.get<RandomAccessIndex>().size() > 0);

  WalletInterface::ReceiverAmounts outputs;
...
// bool WalletInterface::isFusionTransaction(size_t transactionId) const
// {
//   throwIfNotInitialized();
//   throwIfStopped();
...
// bool WalletInterface::isFusionTransaction(const WalletTransaction &walletTx) const
// {
//   if (walletTx.fee != 0)
//   {
//     return false;
...
// IFusionManager::EstimateResult WalletInterface::estimate(uint64_t threshold) const
// {
//   throwIfNotInitialized();
//   throwIfStopped();
...
std::vector<WalletInterface::OutputToTransfer> WalletInterface::pickRandomFusionInputs(uint64_t threshold, size_t minInputCount, size_t maxInputCount)
{
  std::vector<WalletInterface::OutputToTransfer> allFusionReadyOuts;
  auto walletOuts = pickWalletsWithMoney();
  std::array<size_t, std::numeric_limits<uint64_t>::digits10 + 1> bucketSizes;
  bucketSizes.fill(0);
  for (size_t walletIndex = 0; walletIndex < walletOuts.size(); ++walletIndex)
  ...
  std::vector<WalletInterface::OutputToTransfer> selectedOuts;
  ...
  std::vector<WalletInterface::OutputToTransfer> trimmedSelectedOuts;
...
CryptoNote::AccountPublicAddress WalletInterface::getChangeDestination(const std::string &changeDestinationAddress, const std::vector<std::string> &sourceAddresses) const
{
...
bool WalletInterface::isMyAddress(const std::string &addressString) const
{
...
void WalletInterface::deleteContainerFromUnlockTransactionJobs(const ITransfersContainer *container)
{
...
std::vector<size_t> WalletInterface::deleteTransfersForAddress(const std::string &address, std::vector<size_t> &deletedTransactions)
{
...
void WalletInterface::deleteFromUncommitedTransactions(const std::vector<size_t> &deletedTransactions)
{
