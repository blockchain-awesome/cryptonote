...
SpentOutputDescriptor::SpentOutputDescriptor(const TransactionOutputInformationIn& transactionInfo) :
  ...
  if (m_type == output_type_t::Key) {
    m_keyImage = &transactionInfo.keyImage;
  } else if (m_type == output_type_t::Multisignature) {
...
bool TransfersContainer::addTransactionOutputs(const TransactionBlockInfo& block, const ITransactionReader& tx,
  ...
  for (const auto& transfer : transfers) {
    ...
    if (transferIsUnconfirmed) {
      ...
      if (info.type == output_type_t::Multisignature) {
    ...
    if (info.type == output_type_t::Key) {
...
bool TransfersContainer::addTransactionInputs(const TransactionBlockInfo& block, const ITransactionReader& tx) {
  ...
  for (size_t i = 0; i < tx.getInputCount(); ++i) {
    ...
    if (inputType == input_type_t::Key) {
      ...
      inputsAdded = true;
    } else if (inputType == input_type_t::Multisignature) {
      ...
      assert(inputType == input_type_t::Generating);
    }
  }
...
bool TransfersContainer::markTransactionConfirmed(const TransactionBlockInfo& block, const hash_t& transactionHash,
  ...
  for (auto transferIt = availableRange.first; transferIt != availableRange.second; ) {
    ...
    if (transfer.type == output_type_t::Multisignature) {
      SpentOutputDescriptor descriptor(transfer);
      if (m_availableTransfers.get<SpentOutputDescriptorIndex>().count(descriptor) > 0 ||
          m_spentTransfers.get<SpentOutputDescriptorIndex>().count(descriptor) > 0) {
    ...
    if (transfer.type == output_type_t::Key) {
...
void TransfersContainer::deleteTransactionTransfers(const hash_t& transactionHash) {
  ...
  for (auto it = spentTransfersRange.first; it != spentTransfersRange.second;) {
    ...
    if (result.first->type == output_type_t::Key) {
...
bool TransfersContainer::isIncluded(output_type_t type, uint32_t state, uint32_t flags) {
  return
    // filter by type
    (
    ((flags & IncludeTypeKey) != 0            && type == output_type_t::Key) ||
    ((flags & IncludeTypeMultisignature) != 0 && type == output_type_t::Multisignature)
    )
    &&
    // filter by state
    ((flags & state) != 0);
