...
void SynchronizationState::addBlocks(const hash_t* blockHashes, uint32_t height, uint32_t count) {
  assert(blockHashes);
...
const std::vector<hash_t>& SynchronizationState::getKnownBlockHashes() const {
  return m_blockchain;
}
