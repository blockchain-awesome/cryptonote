...
bool TestBlockMajorVersionAccepted::generate(std::vector<test_event_entry>& events) const {
  ...
  bg.generateBlocks(1, config::mainnet::data.block.version.major);
...
bool TestBlockMajorVersionRejected::generate(std::vector<test_event_entry>& events) const {
  TestGenerator bg(m_currency, events);
  bg.generateBlocks(1, config::mainnet::data.block.version.major + 1);
...
bool gen_block_ts_not_checked::generate(std::vector<test_event_entry>& events) const
{
  ...
  generator.constructBlockManually(blk_1, blk_0r, miner_account, 
    test_generator::bf_major_ver | test_generator::bf_timestamp, config::mainnet::data.block.version.major, 0, blk_0.timestamp - 60 * 60);
  events.push_back(blk_1);
...
bool gen_block_ts_in_past::generate(std::vector<test_event_entry>& events) const
{
  ...
  uint64_t ts_below_median = boost::get<block_t>(events[m_currency.timestampCheckWindow() / 2 - 1]).timestamp;
  block_t blk_1;
  generator.constructBlockManually(blk_1, blk_0r, miner_account, 
    test_generator::bf_major_ver | test_generator::bf_timestamp, config::mainnet::data.block.version.major, 0, ts_below_median);
  events.push_back(blk_1);
...
bool gen_block_ts_in_future_rejected::generate(std::vector<test_event_entry>& events) const
{
  ...
  generator.constructBlockManually(blk_1, blk_0, miner_account, test_generator::bf_major_ver | test_generator::bf_timestamp,
    config::mainnet::data.block.version.major, 0, time(NULL) + 60 * 60 + m_currency.blockFutureTimeLimit());
  events.push_back(blk_1);
...
bool gen_block_ts_in_future_accepted::generate(std::vector<test_event_entry>& events) const
{
  ...
  block_t blk_1;
  generator.constructBlockManually(blk_1, blk_0, miner_account, test_generator::bf_major_ver | test_generator::bf_timestamp,
    config::mainnet::data.block.version.major, 0, time(NULL) - 60 + m_currency.blockFutureTimeLimit());
  events.push_back(blk_1);
...
bool gen_block_invalid_prev_id::generate(std::vector<test_event_entry>& events) const
{
  ...
  block_t blk_1;
  hash_t prev_id = Block::getHash(blk_0);
  reinterpret_cast<char &>(prev_id) ^= 1;
  generator.constructBlockManually(blk_1, blk_0, miner_account, 
    test_generator::bf_major_ver | test_generator::bf_prev_id, config::mainnet::data.block.version.major, 0, 0, prev_id);
  events.push_back(blk_1);
...
bool gen_block_invalid_nonce::generate(std::vector<test_event_entry>& events) const
{
  ...
  if (!lift_up_difficulty(m_currency, events, timestamps, commulative_difficulties, generator, 2, blk_0, miner_account,
    config::mainnet::data.block.version.major)) {
    return false;
  }

  // Create invalid nonce
  difficulty_t diffic = m_currency.nextDifficulty(timestamps, commulative_difficulties);
  assert(1 < diffic);
  const block_t& blk_last = boost::get<block_t>(events.back());
  ...
  do
  {
    ...
    if (!generator.constructBlockManually(blk_3, blk_last, miner_account,
      test_generator::bf_major_ver | test_generator::bf_diffic | test_generator::bf_timestamp, config::mainnet::data.block.version.major, 0, timestamp, hash_t(), diffic))
      return false;
  }
  while (0 == blk_3.nonce);
  --blk_3.nonce;
...
bool gen_block_no_miner_tx::generate(std::vector<test_event_entry>& events) const
{
  ...
  block_t blk_1;
  generator.constructBlockManually(blk_1, blk_0, miner_account, 
    test_generator::bf_major_ver | test_generator::bf_miner_tx, config::mainnet::data.block.version.major, 0, 0, hash_t(), 0, miner_tx);
  events.push_back(blk_1);
...
bool gen_block_miner_tx_has_out_to_alice::generate(std::vector<test_event_entry>& events) const
{
  ...
  derive_public_key((const uint8_t*)&derivation, 1, (const uint8_t*)&alice.getAccountKeys().address.spendPublicKey, (uint8_t*)&out_eph_public_key);
...
bool gen_block_has_invalid_tx::generate(std::vector<test_event_entry>& events) const
{
  ...
  std::vector<hash_t> tx_hashes;
  tx_hashes.push_back(hash_t());
...
bool gen_block_invalid_binary_format::generate(std::vector<test_event_entry>& events) const
{
  ...
  std::vector<uint64_t> timestamps;
  std::vector<difficulty_t> cummulative_difficulties;
  difficulty_t cummulative_diff = 1;

  // Unlock blk_0 outputs
  block_t blk_last = blk_0;
  ...
  do
  {
    blk_last = boost::get<block_t>(events.back());
    ...
    if (!lift_up_difficulty(m_currency, events, timestamps, cummulative_difficulties, generator, 1, blk_last,
      miner_account, config::mainnet::data.block.version.major)) {
      return false;
    }
  ...
  blk_last = boost::get<block_t>(events.back());
  MAKE_TX(events, tx_0, miner_account, miner_account, MK_COINS(120), boost::get<block_t>(events[1]));
  ...
  if (!generator.constructBlockManually(blk_test, blk_last, miner_account,
    ...
    config::mainnet::data.block.version.major, 0, blk_last.timestamp, hash_t(), diffic, transaction_t(), tx_hashes, txs_size))
    return false;
