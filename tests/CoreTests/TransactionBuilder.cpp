...
void TransactionBuilder::signSources(const hash_t& prefixHash, const std::vector<cryptonote::key_pair_t>& contexts, transaction_t& tx) const {
  ...
  for (const auto& src_entr : m_sources) {
    ...
    generate_ring_signature((const uint8_t*)&prefixHash, (const uint8_t*)&boost::get<key_input_t>(tx.inputs[i]).keyImage, (const uint8_t *const *)keys_ptrs.data(), keys_ptrs.size(), (const uint8_t*)&contexts[i].secretKey, src_entr.realOutput, (uint8_t *)sigs.data());
    i++;
  }
  ...
  for (const auto& msrc : m_msigSources) {
    ...
    for (const auto& key : msrc.keys) {
      ...
      signature_t sig;
      generate_signature((const uint8_t *)&prefixHash, (const uint8_t *)&ephemeralPublicKey, (const uint8_t *)&ephemeralSecretKey, (uint8_t *)&sig);
