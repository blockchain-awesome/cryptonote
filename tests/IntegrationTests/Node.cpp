...
struct BlockchainInfo {
  ...
  std::unordered_map<hash_t, std::vector<uint32_t>> globalOutputs;
...
void NodeTest::readBlockchainInfo(INode& node, BlockchainInfo& bc) {
  std::vector<hash_t> history = { currency.genesisBlockHash() };
  ...
  do {
    ...
    node.queryBlocks(std::vector<hash_t>(history.rbegin(), history.rend()), timestamp, blocks, startHeight, cb.callback());
