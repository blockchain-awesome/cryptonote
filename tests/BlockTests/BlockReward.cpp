...
namespace
{
  ...
  #define TEST_ALREADY_GENERATED_COINS(alreadyGeneratedCoins, expectedReward)                                  \
    ...
    EXPECT_TRUE(true) << "m_blockTooBig = " << m_blockTooBig << std::endl;                                                            \
  ...
  TEST_F(getBlockReward_and_already_generated_coins, handles_first_values) {
    TEST_ALREADY_GENERATED_COINS(0, 1);
    TEST_ALREADY_GENERATED_COINS(m_blockReward, 3689348814741910528);
  ...
  protected:
    ...
    virtual void SetUp() override {
      ...
      ASSERT_FALSE(m_blockTooBig);
      ASSERT_EQ(UINT64_C(1), m_standardBlockReward);
    }
