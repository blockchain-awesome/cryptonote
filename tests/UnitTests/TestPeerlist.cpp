...
TEST(peer_list, peer_list_general)
{
  ...
  plm.init(false);
#define ADD_GRAY_NODE(ip_, port_, id_, last_seen_) {  peerlist_entry_t ple; ple.last_seen=last_seen_;ple.adr.ip = ip_; ple.adr.port = port_; ple.id = id_;plm.append_with_peer_gray(ple);}  
#define ADD_WHITE_NODE(ip_, port_, id_, last_seen_) {  peerlist_entry_t ple;ple.last_seen=last_seen_; ple.adr.ip = ip_; ple.adr.port = port_; ple.id = id_;plm.append_with_peer_white(ple);}  

#define PRINT_HEAD(step) {std::list<peerlist_entry_t> bs_head; bool r = plm.get_peerlist_head(bs_head, 100);std::cout << "step " << step << ": " << bs_head.size() << std::endl;}
  ...
  std::list<peerlist_entry_t> bs_head;
...
TEST(peer_list, merge_peer_lists)
{
  ...
  PeerlistManager plm;
  plm.init(false);
  std::list<peerlist_entry_t> outer_bs;
