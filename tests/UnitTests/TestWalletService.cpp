...
public:
  ...
  WalletTransactionBuilder& extra(const std::string& extra) {
    transaction.extra = BinaryArray::toString(Common::fromHex(extra));
    return *this;
  }
...
TEST_F(WalletServiceTest_getTransaction, returnsCorrectFields) {
  ...
  ASSERT_EQ(Common::toHex(array::fromString(wallet.transaction.transaction.extra)), transaction.extra);
...
bool isEquivalent(const SendTransaction::Request& request, const TransactionParameters& params) {
  ...
  return std::make_tuple(request.sourceAddresses, orders, request.fee, request.anonymity, extra, request.unlockTime)
      ==
      std::make_tuple(params.sourceAddresses, params.destinations, params.fee, params.mixIn, Common::toHex(array::fromString(params.extra)), params.unlockTimestamp);
}
