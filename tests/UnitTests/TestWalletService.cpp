...
struct IWalletBaseStub : public cryptonote::IWallet {
  ...
  virtual std::string createAddress(const secret_key_t& spendSecretKey) override { return ""; }
  virtual std::string createAddress(const public_key_t& spendPublicKey) override { return ""; }
  ...
  virtual WalletTransactionWithTransfers getTransaction(const hash_t& transactionHash) const override { return WalletTransactionWithTransfers(); }
  virtual std::vector<TransactionsInBlockInfo> getTransactions(const hash_t& blockHash, size_t count) const override { return {}; }
  virtual std::vector<TransactionsInBlockInfo> getTransactions(uint32_t blockIndex, size_t count) const override { return {}; }
  virtual std::vector<hash_t> getBlockHashes(uint32_t blockIndex, size_t count) const override { return {}; }
...
hash_t WalletServiceTest::generateRandomHash() {
  hash_t hash;
...
struct WalletCreateAddressStub: public IWalletBaseStub {
  ...
  virtual std::string createAddress(const secret_key_t& spendSecretKey) override { return address; }
  virtual std::string createAddress(const public_key_t& spendPublicKey) override { return address; }
...
TEST_F(WalletServiceTest_createAddress, correctSecretKey) {
  ...
  std::string address;
  std::error_code ec = service->createAddress(hex::podTo(sec), address);
...
TEST_F(WalletServiceTest_createAddress, correctPublicKey) {
  ...
  std::string address;
  std::error_code ec = service->createTrackingAddress(hex::podTo(pub), address);
...
TEST_F(WalletServiceTest_getSpendKeys, returnsKeysCorrectly) {
  ...
  ASSERT_FALSE(ec);
  ASSERT_EQ(hex::podTo(wallet.keyPair.publicKey), publicSpendKey);
  ASSERT_EQ(hex::podTo(wallet.keyPair.secretKey), secretSpendKey);
}
...
TEST_F(WalletServiceTest_getViewKey, returnsCorrectValue) {
  ...
  ASSERT_EQ(hex::podTo(wallet.keyPair.secretKey), viewSecretKey);
}

class WalletTransactionBuilder {
public:
  WalletTransactionBuilder& hash(const hash_t& hash) {
...
TEST_F(WalletServiceTest_getTransactions, addressesFilter_emptyReturnsTransaction) {
  ...
  ASSERT_EQ(hex::podTo(testTransactions[0].transactions[0].transaction.hash), transactions[0].transactions[0].transactionHash);
}
...
TEST_F(WalletServiceTest_getTransaction, returnsCorrectFields) {
  ...
  TransactionRpcInfo transaction;
  auto ec = service->getTransaction(hex::podTo(hash_t()), transaction);
  ...
  ASSERT_EQ(hex::to(IBinary::from(wallet.transaction.transaction.extra)), transaction.extra);
  ...
  ASSERT_EQ(hex::podTo(wallet.transaction.transaction.hash), transaction.transactionHash);
...
struct WalletTransferStub : public IWalletBaseStub {
  WalletTransferStub(System::Dispatcher& dispatcher, const hash_t& hash) : IWalletBaseStub(dispatcher), hash(hash) {
  }
...
bool isEquivalent(const SendTransaction::Request& request, const TransactionParameters& params) {
  ...
  return std::make_tuple(request.sourceAddresses, orders, request.fee, request.anonymity, extra, request.unlockTime)
      ==
      std::make_tuple(params.sourceAddresses, params.destinations, params.fee, params.mixIn, hex::to(IBinary::from(params.extra)), params.unlockTimestamp);
}
...
//   ASSERT_FALSE(ec);
//   ASSERT_EQ(hex::podTo(wallet.hash), hash);
...
struct WalletMakeTransactionStub : public IWalletBaseStub {
  WalletMakeTransactionStub(System::Dispatcher& dispatcher, const hash_t& hash) : IWalletBaseStub(dispatcher), hash(hash) {
  }
...
struct WalletGetDelayedTransactionIdsStub : public IWalletBaseStub {
  WalletGetDelayedTransactionIdsStub(System::Dispatcher& dispatcher, const hash_t& hash) : IWalletBaseStub(dispatcher), hash(hash) {
  }
...
TEST_F(WalletServiceTest_getDelayedTransactionHashes, returnsCorrectResult) {
  ...
  ASSERT_EQ(hex::podTo(wallet.hash), hashes[0]);
}
...
TEST_F(WalletServiceTest_getUnconfirmedTransactionHashes, returnsAllHashesWithoutAddresses) {
  ...
  ASSERT_EQ(hashes[0], hex::podTo(transactions[0].transaction.hash));
  ASSERT_EQ(hashes[1], hex::podTo(transactions[1].transaction.hash));
}
