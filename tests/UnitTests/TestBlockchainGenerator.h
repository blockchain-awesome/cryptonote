...
public:
  ...
  void addTxToBlockchain(const transaction_t& transaction);
  bool getTransactionByHash(const hash_t& hash, transaction_t& tx, bool checkTxPool = false);
  const cryptonote::Account& getMinerAccount() const;
  bool generateFromBaseTx(const cryptonote::Account& address);

  void putTxToPool(const transaction_t& tx);
...
private:
  ...
  std::unordered_map<hash_t, transaction_t> m_txs;
  ...
  std::unordered_map<hash_t, transaction_t> m_txPool;
  ...
  void addToBlockchain(const transaction_t& tx);
  void addToBlockchain(const std::vector<transaction_t>& txs);
  void addToBlockchain(const std::vector<transaction_t>& txs, const cryptonote::Account& minerAddress);
  void addTx(const transaction_t& tx);
