...
TEST_F(InProcessNodeTests, getRandomOutsByAmountsSuccess) {
  public_key_t ignoredPublicKey;
  secret_key_t ignoredSectetKey;
  generate_keys((uint8_t*)&ignoredPublicKey, (uint8_t*)&ignoredSectetKey);
...
TEST_F(InProcessNodeTests, getNewBlocksUninitialized) {
  cryptonote::InProcessNode newNode(coreStub, protocolQueryStub);
  std::vector<hash_t> knownBlockIds;
...
TEST_F(InProcessNodeTests, getTransactionOutsGlobalIndicesUninitialized) {
  ...
  CallbackStatus status;
  newNode.getTransactionOutsGlobalIndices(hash_t(), outsGlobalIndices, [&] (std::error_code ec) { status.setStatus(ec); });
  ASSERT_TRUE(status.wait());
  ASSERT_NE(std::error_code(), status.getStatus());
}
...
TEST_F(InProcessNodeTests, getBlocksByHeightMany) {
  ...
  for (const boost::tuple<boost::tuple<size_t, cryptonote::block_t>, std::vector<cryptonote::block_details_t>>& sameHeight : range) {
    ...
    for (const cryptonote::block_details_t& block : sameHeight.get<1>()) {
      ...
      hash_t expectedCryptoHash = cryptonote::Block::getHash(sameHeight.get<0>().get<1>());
...
TEST_F(InProcessNodeTests, getBlocksByHashMany) {
  ...
  auto range = boost::combine(range1, actualBlocks);
  for (const boost::tuple<boost::tuple<hash_t, cryptonote::block_t>, cryptonote::block_details_t>& sameHeight : range) {
    hash_t expectedCryptoHash = cryptonote::Block::getHash(sameHeight.get<0>().get<1>());
...
TEST_F(InProcessNodeTests, getTxMany) {
  ...
  for (size_t i = 0; i < POOL_TX_NUMBER; ++i) {
    ...
    transactionHashes.push_back(cryptonote::BinaryArray::objectHash(tx));
    coreStub.addTransaction(tx);
    expectedTransactions.push_back(std::make_tuple(tx, boost::value_initialized<hash_t>(), boost::value_initialized<uint64_t>()));
  }
  ...
  for (const boost::tuple<boost::tuple<hash_t, cryptonote::transaction_details_t>, std::tuple<transaction_t, hash_t, uint64_t>>& sameHeight : range) {
    ...
    if (std::get<1>(sameHeight.get<1>()) != boost::value_initialized<hash_t>()) {
...
TEST_F(InProcessNodeTests, getPoolDiffereceNotInited) {
  ...
  std::vector<hash_t> knownPoolTxIds; 
  hash_t knownBlockId = boost::value_initialized<hash_t>();
...
TEST_F(InProcessNodeTests, getPoolDiffereceActualBC) {
  ...
  std::unordered_set<hash_t> transactionHashes;
  ...
  std::vector<hash_t> knownPoolTxIds;
  hash_t knownBlockId = cryptonote::BinaryArray::objectHash(generator.getBlockchain().back());
  bool isBcActual = false;
  std::vector<std::unique_ptr<ITransactionReader>> newTxs;
  std::vector<hash_t> deletedTxIds;
